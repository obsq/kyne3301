...
async def kang(args):
    ...
    if photo:
        ...
        if "  A <strong>Telegram</strong> user has created the <strong>Sticker&nbsp;Set</strong>." not in htmlstr:
            ...
            await args.edit(f"`{KYNE_NNAME}`: **Brewing a new Pack...**")
            async with bot.conversation('Stickers') as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
        ...
        await args.edit(f"`{KYNE_NNAME}`: **Sticker kanged successfully!**\
            \nPack can be found [here](t.me/addstickers/{packname})",
                        parse_mode='md')
...
@kyne3301(outgoing=True, disable_errors=True, pattern="^!ss2(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return 
    if not event.reply_to_msg_id:
       await event.edit(f"`{KYNE_NNAME}`: **Reply to a Photo**")
       return
    ...
    if reply_message.sender.bot:
       ...
       return
    await event.edit(f"`{KYNE_NNAME}`: **processing Img to Sticker.......**")
    async with bot.conversation(chat) as conv:
          try:                
              ...
              if response2.text.startswith("🔸"):      
                 ...
                 #await event.reply(f"`{KYNE_NNAME}`: **Converted @{sender.username}'s photo to Sticker Sucessfully**")
                 if response.text.startswith("🔸"):               
                    await event.edit(f"`{KYNE_NNAME}`: **Sorry, Failed to decode this img \n try use alternative way !ss3**")
                 else: 
...
@kyne3301(outgoing=True, pattern="^\!stickerinfo$")
async def get_pack_info(event):
    if not event.is_reply:
        await event.edit(f"`{KYNE_NNAME}`: **Reply to a sticker to get the pack details*")
        return
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        await event.edit(f"`{KYNE_NNAME}`: **This is not a sticker. Reply to a sticker.**")
        return
...
@kyne3301(outgoing=True, pattern="^!ss(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return 
    if not event.reply_to_msg_id:
    ...
    if reply_message.sender.bot:
       await event.edit(f"`{KYNE_NNAME}`: **Reply to actual users message**")
       return
    await event.edit(f"`{KYNE_NNAME}`: ** Processing user chat to Sticker **")
    async with bot.conversation(chat) as conv:
          try:                
              await conv.send_message("/start")
              response2 = await conv.get_response()     
...
@kyne3301.on(rekcah05(pattern=f"ss2(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return 
    if not event.reply_to_msg_id:
       await event.reply(f"`{KYNE_NNAME}`: **Reply to a Photo**")
       return
    reply_message = await event.get_reply_message() 
    if not reply_message.media:
       await event.reply(f"`{KYNE_NNAME}`: **Reply to a photo**")
    ...
    if reply_message.sender.bot:
       ...
       return
    await event.reply(f"`{KYNE_NNAME}`: **processing Img to Sticker.......**")
    async with bot.conversation(chat) as conv:
          try:                
              await conv.send_message("/start")
              response2 = await conv.get_response()     
              if response2.text.startswith("🔸"):      
                 ...
                 #await event.reply(f"`{KYNE_NNAME}`: **Converted @{sender.username}'s photo to Sticker Sucessfully**")
                 if response.text.startswith("🔸"):               
                    await event.reply(f"`{KYNE_NNAME}`: **Sorry, Failed to decode this img \n try use alternative way !ss3**")
                 else: 
          	       await bot.send_file(event.chat_id, response.message.media)      
              else:
