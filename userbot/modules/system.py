...
@kyne3301(outgoing=True, pattern="^\!speed$")
...
@kyne3301(outgoing=True, pattern="^\!dc$")
...
@kyne3301(outgoing=True, pattern="^\!ping$")
...
@kyne3301(pattern=r"^\!download(?: |$)(.*)", outgoing=True)
...
@kyne3301(pattern=r"^\!uploadir (.*)", outgoing=True)
...
@kyne3301(pattern=r"^\!upload (.*)", outgoing=True)
...
@kyne3301(pattern=r"^\!uploadas(stream|vn|all) (.*)", outgoing=True)
...
@kyne3301(outgoing=True, pattern="^\!random")
...
@kyne3301(outgoing=True, pattern="^\!sleep( [0-9]+)?$")
...
@kyne3301(outgoing=True, pattern="^\!shutdown$")
...
@kyne3301(outgoing=True, pattern="^\!restart$")
async def killdabot(event):
...
@kyne3301(outgoing=True, pattern="^\!repeat (.*)")
...
@kyne3301(outgoing=True, pattern="^\!repo$")
async def repo_is_here(wannasee):
    """ For .repo command, just returns the repo URL. """
    await wannasee.edit(
        f"This bot just re-edit PaperplaneExtended  and added more features from uniborg's plugin\n Click [here](https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Frekcah-pavi%2Fjaves&template=https%3A%2F%2Fgithub.com%2Frekcah-pavi%2Fjaves) to open kyne\n =>Join channel for more information @kyne3301")


@kyne3301(outgoing=True, pattern="^\!raw$")
async def raw(event):
    ...
    with io.BytesIO(str.encode(the_real_message)) as out_file:
        ...
        await event.edit(
            f"`{KYNE_NNAME}:  `i have sent decoded message data in my chat!")
...
@kyne3301(outgoing=True, pattern=r"^!(set|get|del) var(?: |$)(.*)(?: |$)")
...
@kyne3301(outgoing=True, pattern=r"^!usage(?: |$)")
...
@kyne.on(admin_cmd(pattern=f"sysd$", allow_sudo=True))
async def sysdetails(sysd):
    """ For .sysd command, get system info using neofetch. """
    try:
        neo = "neofetch --stdout"
...
@kyne.on(admin_cmd(pattern=f"pip(?: |$)(.*)", allow_sudo=True))
async def pipcheck(pip):
    """ For .pip command, do a pip search. """
    pipmodule = pip.pattern_match.group(1)
    if pipmodule:
...
@kyne.on(admin_cmd(pattern=f"sleep( [0-9]+)?$", allow_sudo=True))
async def sleepybot(time):
    """ For .sleep command, let the userbot snooze for a few second. """
    message = time.text
    if " " not in time.pattern_match.group(1):
...
@kyne.on(admin_cmd(pattern=f"shutdown$", allow_sudo=True))
async def killdabot(event):
    """ For .shutdown command, shut the bot down."""
...
@kyne.on(admin_cmd(pattern=f"repeat (.*)", allow_sudo=True))
async def repeat(rep):
    cnt, txt = rep.pattern_match.group(1).split(' ', 1)
    replyCount = int(cnt)
    toBeRepeated = txt
...
@kyne.on(admin_cmd(pattern=f"repo$", allow_sudo=True))
async def repo_is_here(wannasee):
    """ For .repo command, just returns the repo URL. """
...
@kyne.on(admin_cmd(pattern=f"raw$", allow_sudo=True))
async def repo_is_here(wannasee):
    """ For .repo command, just returns the repo URL. """
    await wannasee.reply(
...
@kyne.on(admin_cmd(pattern=f"(set|get|del) var(?: |$)(.*)(?: |$)", allow_sudo=True))
async def repo_is_here(wannasee):
    """ For .repo command, just returns the repo URL. """
    await wannasee.reply(
        f"`Privacy Error\nSorry this command not permitted`")

@kyne.on(admin_cmd(pattern=f"usage(?: |$)", allow_sudo=True))
async def dyno_usage(dyno):
    """
        Get your account Dyno Usage
    """
...
@kyne.on(admin_cmd(pattern=f"restart$", allow_sudo=True))
async def killdabot(event):
...
@kyne.on(admin_cmd(pattern=f"random", allow_sudo=True))
async def randomise(items):
    """ For .random command, get a random item from the list of items. """
    itemo = (items.text[8:]).split()
    if len(itemo) < 2:
...
@kyne.on(admin_cmd(pattern=f"upload (.*)", allow_sudo=True))
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the userbot's server """
...
@kyne.on(admin_cmd(pattern=f"uploadir (.*)", allow_sudo=True))
async def uploadir(udir_event):
    """ For .uploadir command, allows you to upload everything from a folder in the server"""
    input_str = udir_event.pattern_match.group(1)
    if os.path.exists(input_str):
...
@kyne.on(admin_cmd(pattern=f"download(?: |$)(.*)", allow_sudo=True))
async def download(target_file):
    """ For .download command, download files to the userbot's server. """
...
@kyne.on(admin_cmd(pattern=f"ping$", allow_sudo=True))
async def pingme(pong):
    """ For .ping command, ping the userbot from any chat.  """
    start = datetime.now()
...
@kyne.on(admin_cmd(pattern=f"speed$", allow_sudo=True))
async def speedtst(spd):
    """ For .speed command, use SpeedTest to check server speeds. """
...
@kyne.on(admin_cmd(pattern=f"update(?: |$)(.*)", allow_sudo=True))
async def upstream(ups):
    "For .update command, check if the bot is up to date, update if specified"
