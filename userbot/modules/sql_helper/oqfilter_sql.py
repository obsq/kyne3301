...
class oqfilters(BASE):
    __tablename__ = "oqfilters"
...
oqfilters.__table__.create(checkfirst=True)


def get_filter(chat_id, keyword):
    try:
        return SESSION.query(oqfilters).get((str(chat_id), keyword))
    except:
        return None
    finally:
        SESSION.close()
...
def add_filter(chat_id, keyword, reply, snip_type, media_id, media_access_hash, media_file_reference):
    ...
    if adder:
        ...
        adder = oqfilters(chat_id, keyword, reply, snip_type, media_id,
...
def remove_all_oqfilters(chat_id):
    saved_filter = SESSION.query(oqfilters).filter(oqfilters.chat_id == str(chat_id))
    if saved_filter:
        saved_filter.delete()
        SESSION.commit()
